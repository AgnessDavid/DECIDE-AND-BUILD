...
@if(in_array($mode, ['mobile_money', 'paypal', 'stripe', 'carte']))
    <form action="{{ route('afficher.paiement', $commande->id ?? 0) }}" method="POST" class="space-y-4 mt-4 bg-white p-4 rounded-lg shadow-sm">
        ...
        <button type="submit"
            ...
            Payer {{ number_format($commande->total_ttc ?? 0, 2, ',', ' ') }} FCFA
        </button>
    </form>
@else
    <p class="text-gray-700 text-center mt-4">Le mode de paiement sélectionné ne nécessite pas de traitement en ligne.</p>
@endif
